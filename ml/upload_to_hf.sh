...
if command -v hf &> /dev/null; then
    ...
    echo "❌ Neither 'hf' nor 'huggingface-cli' found. Install with: pip install huggingface_hub"
    exit 1
fi
...
$HF_CMD repo create "$(basename "$REPO_ID")" --type model 2>/dev/null || true

# Upload the model card as README.md
echo "📄 Uploading model card..."
$HF_CMD upload "$REPO_ID" "$MODEL_CARD" README.md --repo-type model

# Upload the encoder weights
echo "🧠 Uploading swipe_encoder.bin ($(du -h "$ASSETS_DIR/swipe_encoder.bin" | cut -f1))..."
$HF_CMD upload "$REPO_ID" "$ASSETS_DIR/swipe_encoder.bin" swipe_encoder.bin --repo-type model

# Upload the lexicon
echo "📖 Uploading swipe_words.txt ($(du -h "$ASSETS_DIR/swipe_words.txt" | cut -f1))..."
$HF_CMD upload "$REPO_ID" "$ASSETS_DIR/swipe_words.txt" swipe_words.txt --repo-type model
